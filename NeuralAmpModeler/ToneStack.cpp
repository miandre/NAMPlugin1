...
namespace
{
double GetAmp2PresenceGainDB(const double val)
{
  return (val >= 5.0) ? (1.2 * (val - 5.0)) : (2.0 * (val - 5.0));
}
} // namespace
...
DSP_SAMPLE** dsp::tone_stack::Amp2ToneStack::Process(DSP_SAMPLE** inputs, const int numChannels, const int numFrames)
{
  ...
  DSP_SAMPLE** midPointers = mToneMid.Process(bassPointers, numChannels, numFrames);
  DSP_SAMPLE** treblePointers = mToneTreble.Process(midPointers, numChannels, numFrames);
  DSP_SAMPLE** trebleTiltPointers = mAmp2TrebleTiltComp.Process(treblePointers, numChannels, numFrames);
  DSP_SAMPLE** presencePointers = mTonePresence.Process(trebleTiltPointers, numChannels, numFrames);
  DSP_SAMPLE** amp2DepthPointers = mAmp2DepthBoost.Process(presencePointers, numChannels, numFrames);
...
void dsp::tone_stack::Amp2ToneStack::SetParam(const std::string name, const double val)
{
  if (name == "bass")
  {
    mBassVal = val;
    const double sampleRate = GetSampleRate();
    const double bassGainDB = 1.2 * (val - 5.0); // +/- 6
    const double bassFrequency = 130.0;
    const double bassQuality = 0.9;
    recursive_linear_filter::BiquadParams bassParams(sampleRate, bassFrequency, bassQuality, bassGainDB);
    mToneBass.SetParams(bassParams);
  }
  else if (name == "middle")
  {
    mMiddleVal = val;
    const double sampleRate = GetSampleRate();
    const double midGainDB = 0.8 * (val - 5.0); // +/- 4
    const double midFrequency = 660.0;
    const double midQuality = 0.6;
    recursive_linear_filter::BiquadParams midParams(sampleRate, midFrequency, midQuality, midGainDB);
    mToneMid.SetParams(midParams);
  }
  else if (name == "treble")
  {
    mTrebleVal = val;
    const double sampleRate = GetSampleRate();
    const double trebleGainDB = 1.0 * (val - 5.0); // +/- 5
    const double trebleFrequency = 4000.0;
    const double trebleQuality = 0.40;
    recursive_linear_filter::BiquadParams trebleParams(sampleRate, trebleFrequency, trebleQuality, trebleGainDB);
    mToneTreble.SetParams(trebleParams);

    const double tiltCompGainDB = -0.2 * (val - 5.0); // Subtle inverse low tilt, about +/- 1 dB.
    const double tiltCompFrequency = 400.0;
    const double tiltCompQuality = 0.7;
    recursive_linear_filter::BiquadParams tiltCompParams(
      sampleRate, tiltCompFrequency, tiltCompQuality, tiltCompGainDB);
    mAmp2TrebleTiltComp.SetParams(tiltCompParams);
  }
  else if (name == "presence")
  {
    mPresenceVal = val;
    const double sampleRate = GetSampleRate();
    const double presenceGainDB = GetAmp2PresenceGainDB(val);
    const double presenceFrequency = 3500.0;
    const double presenceQuality = 0.8;
    recursive_linear_filter::BiquadParams presenceParams(
      sampleRate, presenceFrequency, presenceQuality, presenceGainDB);
    mTonePresence.SetParams(presenceParams);
  }
  else if (name == "amp2_depth_button")
  {
    mAmp2DepthButtonVal = val;
    const double sampleRate = GetSampleRate();
    const bool enabled = val >= 0.5;
    const double depthBoostGainDB = enabled ? 8.0 : 0.0;
    const double depthBoostFrequency = 80.0;
    const double depthBoostQuality = 0.9;
  ...
  else if (name == "amp2_scoop_button")
  {
    ...
    const double sampleRate = GetSampleRate();
    const bool enabled = val >= 0.5;
    // Keep Scoop distinct from the Mid control by centering it higher and wider.
    const double scoopGainDB = enabled ? -4.5 : 0.0;
    const double scoopFrequency = 850.0;
    ...
    const double makeupGainDB = enabled ? 0.75 : 0.0;
